...
class Net6(Gen):  # syntax ex. fec0::/126
    ...
    def __init__(self, net):
        ...
        if not self.ipaddress.match(net):
            tmp[0] = socket.getaddrinfo(
                tmp[0], None, socket.AF_INET6)[0][-1][0]
    ...
    def __iter__(self):
        ...
        def parse_digit(a, netmask):
            ...
            a = (int(a) & (0xffL << netmask),
                 (int(a) | (0xffL >> (8 - netmask))) + 1)
            return a
        self.parsed = map(lambda x, y: parse_digit(x, y), tmp,
                          map(lambda x, nm=self.plen: x - nm, tuple))
...
class _HopByHopOptionsField(PacketListField):
    ...
    def __init__(self, name, default, cls, curpos, count_from=None, length_from=None):
        ...
        PacketListField.__init__(
            self, name, default, cls, count_from=count_from, length_from=length_from)
...
class IPv6ExtHdrHopByHop(_IPv6ExtHdr):
    ...
    fields_desc = [ByteEnumField("nh", 59, ipv6nh),
                   ...
                   # autopad activated by default
                   _PhantomAutoPadField("autopad", 1),
...
class IPv6ExtHdrRouting(_IPv6ExtHdr):
    ...
    fields_desc = [ByteEnumField("nh", 59, ipv6nh),
                   ...
                   # There is meaning in this field ...
...
def defragment6(pktlist):
    ...
    l = filter(lambda x: IPv6ExtHdrFragment in x,
               pktlist)  # remove non fragments
    if not l:
        return []
    ...
    for p in res:
        ...
        if offset != len(fragmentable):
            warning("Expected an offset of %d. Found %d. Padding with XXXX" %
                    (len(fragmentable), offset))
...
class TruncPktLenField(PacketLenField):
    ...
    def __init__(self, name, default, cls, cur_shift, length_from=None, shift=0):
        PacketLenField.__init__(self, name, default,
                                cls, length_from=length_from)
...
class DomainNameListField(StrLenField):
    ...
    def i2m(self, pkt, x):
        ...
        ret_string = "".join(
            map(lambda x: conditionalTrailingDot("".join(x)), tmp))

        # In padded mode, add some \x00 bytes
        if self.padded and not len(ret_string) % self.padded_unit == 0:
            ret_string += "\x00" * \
                (self.padded_unit - len(ret_string) % self.padded_unit)
...
class MIP6OptCGAParams(_MIP6OptAlign, Packet):  # RFC 4866 (Sect. 5.1)
    ...
    fields_desc = [ByteEnumField("otype", 12, _mobopttypes),
                   FieldLenField(
        "olen", None, length_of="cgaparams", fmt="B"),
...
class MIP6MH_BU(_MobilityHeader):
    ...
    fields_desc = [ByteEnumField("nh", 59, ipv6nh),
                   # unit == 8 bytes (excluding the first 8 bytes)
                   ByteField("len", None),
...
class L3RawSocket6(L3RawSocket):
    ...
    def __init__(self, type=ETH_P_IPV6, filter=None, iface=None, promisc=None, nofilter=0):
        ...
        # NOTE: if fragmentation is needed, it will be done by the kernel (RFC
        # 2292)
