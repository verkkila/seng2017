    ...
    def any2i(self, pkt, val):
        if type(val) is str:
            ...
            if hmsms:
                ...
                ms = int(((ms or "") + "000")[:3])
                val = ((int(h) * 60 + int(m or 0)) *
                       60 + int(s or 0)) * 1000 + ms
            else:
                val = 0
        elif val is None:
    ...
    def post_build(self, p, pay):
        ...
        if self.chksum is None:
            if isinstance(self.underlayer, IP):
                ...
                ck = scapy.layers.inet6.in6_chksum(
                    socket.IPPROTO_UDP, self.underlayer, p)
                # According to RFC2460 if the result checksum is 0, it should
                # be set to 0xFFFF
                if ck == 0:
                    ck = 0xFFFF
                p = p[:6] + struct.pack("!H", ck) + p[8:]
...
class ICMP(Packet):
    ...
    fields_desc = [ByteEnumField("type", 8, icmptypes),
                   ...
                   XShortField("chksum", None),
                   ConditionalField(XShortField("id", 0), lambda pkt:pkt.type in [
                                    0, 8, 13, 14, 15, 16, 17, 18]),
                   ConditionalField(XShortField("seq", 0), lambda pkt:pkt.type in [
                                    0, 8, 13, 14, 15, 16, 17, 18]),
                   ConditionalField(ICMPTimeStampField(
                       "ts_ori", None), lambda pkt:pkt.type in [13, 14]),
                   ConditionalField(ICMPTimeStampField(
                       "ts_rx", None), lambda pkt:pkt.type in [13, 14]),
                   ConditionalField(ICMPTimeStampField(
                       "ts_tx", None), lambda pkt:pkt.type in [13, 14]),
                   ConditionalField(IPField("gw", "0.0.0.0"),
                                    lambda pkt:pkt.type == 5),
                   ConditionalField(ByteField("ptr", 0),
                                    lambda pkt:pkt.type == 12),
                   ConditionalField(ByteField("reserved", 0),
                                    lambda pkt:pkt.type in [3, 11]),
                   ConditionalField(ByteField("length", 0),
                                    lambda pkt:pkt.type in [3, 11, 12]),
                   ConditionalField(IPField("addr_mask", "0.0.0.0"),
                                    lambda pkt:pkt.type in [17, 18]),
                   ConditionalField(ShortField("nexthopmtu", 0),
                                    lambda pkt:pkt.type == 3),
                   ConditionalField(ShortField("unused", 0),
                                    lambda pkt:pkt.type in [11, 12]),
                   ConditionalField(IntField("unused", 0), lambda pkt:pkt.type not in [
                                    0, 3, 5, 8, 11, 12, 13, 14, 15, 16, 17, 18])
                   ]
...
def _packetlist_timeskew_graph(self, ip, **kargs):
    ...
    b = filter(lambda x: x.haslayer(IP) and x.getlayer(
        IP).src == ip and x.haslayer(TCP), res)
    ...
    def _wrap_data(ts_tuple, wrap_seconds=2000):
        ...
        X = ct % wrap_seconds
        Y = ((ct - first_creation_time) -
             ((rt - first_replied_timestamp) / 1000.0))
...
PacketList.timeskew_graph = new.instancemethod(
    _packetlist_timeskew_graph, None, PacketList)
...
class TracerouteResult(SndRcvList):
    ...
    def trace3D(self):
        ...
        class IPsphere(visual.sphere):
            ...
            def setlabel(self, txt, visible=None):
                ...
                self.label = visual.label(
                    text=txt, pos=self.pos, space=self.radius, xoffset=10, yoffset=20, visible=visible)
        ...
        for trlst in tr3d.itervalues():
            ...
            start = (0, 0, 0)
            for ip in trlst:
                visual.cylinder(pos=start, axis=ip.pos -
                                start, color=col, radius=0.2)
                start = ip.pos
                ...
                if ev.press == "left":
                    ...
                    if o:
                        if ev.ctrl:
                            ...
                            o.color = (1, 0, 0)
                            a, b = sr(
                                IP(dst=o.ip) / TCP(dport=[21, 22, 23, 25, 80, 443]), timeout=2)
                            o.color = savcolor
                            if len(a) == 0:
                                txt = "%s:\nno results" % o.ip
                            else:
                                ...
                                for s, r in a:
                                    txt += r.sprintf(
                                        "{TCP:%IP.src%:%TCP.sport% %TCP.flags%}{TCPerror:%IPerror.dst%:%TCPerror.dport% %IP.src% %ir,ICMP.type%}\n")
    ...
    def make_graph(self, ASres=None, padding=0):
        ...
        ports_done = {}
        for s, r in self.res:
            r = r.getlayer(IP) or (conf.ipv6_enabled and r[
                scapy.layers.inet6.IPv6]) or r
            s = s.getlayer(IP) or (conf.ipv6_enabled and s[
                scapy.layers.inet6.IPv6]) or s
            ips[r.src] = None
            if TCP in s:
            ...
            if not (ICMP in r and r[ICMP].type == 11) and not (conf.ipv6_enabled and scapy.layers.inet6.IPv6 in r and scapy.layers.inet6.ICMPv6TimeExceeded in r):
                ...
                if TCP in r:
                    p.append(
                        r.sprintf("<T%ir,TCP.sport%> %TCP.sport% %TCP.flags%"))
                    ...
                    p.append(
                        r.sprintf("{IP:<P%ir,proto%> IP %proto%}{IPv6:<P%ir,nh%> IPv6 %nh%}"))
                    trace[ttl] = r.sprintf(
                        '"%r,src%":{IP:P%ir,proto%}{IPv6:P%ir,nh%}')
                ports[r.src] = p
            else:
...
report_ports(target, ports) -> string"""
    ...
    for s, r in ans:
        if r.haslayer(ICMP):
            rep += r.sprintf(
                "%TCPerror.dport% & closed & ICMP type %ICMP.type%/%ICMP.code% from %IP.src% \\\\\n")
...
def IPID_count(lst, funcID=lambda x: x[1].id, funcpres=lambda x: x[1].summary()):
    ...
    idlst.sort()
    classes = [idlst[0]] + map(lambda x: x[1], filter(lambda (x, y): abs(
        x - y) > 50, map(lambda x, y: (x, y), idlst[:-1], idlst[1:])))
...
#    pkt = IP(dst=target, id=RandShort(), options="\x22"*40)/UDP()/load
    pkt = IP(dst=target, id=RandShort(), options="\x00" * 40,
             flags=1) / UDP(sport=sport, dport=sport) / load
    s = conf.L3socket()
    intr = 0
    found = {}
    try:
...
def fragleak2(target, timeout=0.4, onlyasc=0):
    ...
    try:
        while 1:
            p = sr1(IP(dst=target, options="\x00" * 40, proto=200) /
                    "XXXXYYYYYYYYYYYY", timeout=timeout, verbose=0)
            if not p:
                continue
            if conf.padding_layer in p:
