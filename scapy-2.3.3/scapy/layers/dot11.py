...
try:
    ...
    log_loading.info(
        "Can't import python Crypto lib. Won't be able to decrypt WEP.")
...
class PPI(Packet):
    ...
    fields_desc = [ByteField("version", 0),
                   ByteField("flags", 0),
                   FieldLenField("len", None, fmt="<H",
                                 length_of="fields", adjust=lambda pkt, x:x + 8),
                   LEIntField("dlt", 0),
...
class Dot11(Packet):
    ...
    fields_desc = [
        ...
        BitEnumField("type", 0, 2, ["Management",
                                    "Control", "Data", "Reserved"]),
        BitField("proto", 0, 2),
...
class Dot11PacketList(PacketList):
    ...
    def toEthernet(self):
        data = map(lambda x: x.getlayer(Dot11), filter(
            lambda x: x.haslayer(Dot11) and x.type == 2, self.res))
        r2 = []
        for p in data:
            q = p.copy()
            q.unwep()
