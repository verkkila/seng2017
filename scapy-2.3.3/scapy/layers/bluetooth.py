...
class HCI_Hdr(Packet):
    ...
    fields_desc = [ByteEnumField(
        "type", 2, {1: "command", 2: "ACLdata", 3: "SCOdata", 4: "event", 5: "vendor"}), ]
...
class L2CAP_ConnResp(Packet):
    ...
    fields_desc = [LEShortField("dcid", 0),
                   ...
                   LEShortEnumField("result", 0, [
                                    "success", "pend", "cr_bad_psm", "cr_sec_block", "cr_no_mem", "reserved", "cr_inval_scid", "cr_scid_in_use"]),
                   LEShortEnumField(
...
class L2CAP_ConfResp(Packet):
    ...
    fields_desc = [LEShortField("scid", 0),
                   LEShortField("flags", 0),
                   LEShortEnumField(
                       "result", 0, ["success", "unaccept", "reject", "unknown"]),
                   ]
...
class SM_Pairing_Request(Packet):
    ...
    fields_desc = [ByteEnumField("iocap", 3, {0: "DisplayOnly", 1: "DisplayYesNo", 2: "KeyboardOnly", 3: "NoInputNoOutput", 4: "KeyboardDisplay"}),
                   ByteEnumField(
        "oob", 0, {0: "Not Present", 1: "Present (from remote device)"}),
...
class HCI_Cmd_Set_Event_Mask(Packet):
    name = "Set Event Mask"
    fields_desc = [StrFixedLenField(
        "mask", "\xff\xff\xfb\xff\x07\xf8\xbf\x3d", 8)]
...
class HCI_Cmd_LE_Set_Advertising_Parameters(Packet):
    ...
    fields_desc = [LEShortField("interval_min", 0x0800),
                   ...
                   ByteEnumField("adv_type", 0, {
                                 0: "ADV_IND", 1: "ADV_DIRECT_IND", 2: "ADV_SCAN_IND", 3: "ADV_NONCONN_IND", 4: "ADV_DIRECT_IND_LOW"}),
...
bind_layers(HCI_Command_Hdr,
            HCI_Cmd_LE_Set_Advertising_Parameters, opcode=0x2006)
...
bind_layers(HCI_Command_Hdr,
            HCI_Cmd_LE_Long_Term_Key_Request_Reply, opcode=0x201a)
bind_layers(HCI_Command_Hdr,
            HCI_Cmd_LE_Long_Term_Key_Request_Negative_Reply, opcode=0x201b)
...
bind_layers(HCI_Event_Command_Complete,
            HCI_Cmd_Complete_Read_BD_Addr, opcode=0x1009)
...
class BluetoothHCISocket(SuperSocket):
    ...
    def __init__(self, iface=0x10000, type=None):
        ...
        s.setsockopt(socket.SOL_HCI, socket.HCI_FILTER, struct.pack(
            "IIIh2x", 0xffffffffL, 0xffffffffL, 0xffffffffL, 0))  # type mask, event mask, event mask, opcode
        s.bind((iface,))
        self.ins = self.outs = s
#        s.connect((peer,0))
