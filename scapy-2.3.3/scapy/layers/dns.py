        ...
        if pkt.type == 1:  # A
            ...
            while tmp_s:
                ...
                if tmp_len > len(tmp_s):
                    warning(
                        "DNS RR TXT prematured end of character-string (size=%i, remaining bytes=%i)" % (tmp_len, len(tmp_s)))
...
dnssecdigesttypes = {0: "Reserved", 1: "SHA-1",
...
def RRlist2bitmap(lst):
    ...
    for wb in xrange(min_window_blocks, max_window_blocks + 1):
        ...
        for tmp in xrange(bytes):
            ...
            tmp_rrlist = filter(lambda x: 256 * wb + 8 *
                                tmp <= x < 256 * wb + 8 * tmp + 8, rrlist)
            if not tmp_rrlist == []:
                # 1. rescale to fit into 8 bits
                tmp_rrlist = map(lambda x: (x - 256 * wb) -
                                 (tmp * 8), tmp_rrlist)
...
class DNSRRNSEC3(_DNSRRdummy):
    ...
    fields_desc = [DNSStrField("rrname", ""),
                   ...
                   StrLenField(
        "salt", "", length_from=lambda x: x.saltlength),
        FieldLenField("hashlength", 0, fmt="!B",
                      length_of="nexthashedownername"),
        StrLenField("nexthashedownername", "",
                    length_from=lambda x: x.hashlength),
        RRlistField("typebitmaps", "")
    ]
...
class DNSRRNSEC3PARAM(_DNSRRdummy):
    ...
    fields_desc = [DNSStrField("rrname", ""),
                   ...
                   FieldLenField("saltlength", 0, fmt="!B", length_of="salt"),
                   StrLenField(
        "salt", "", length_from=lambda pkt: pkt.saltlength)
    ]
