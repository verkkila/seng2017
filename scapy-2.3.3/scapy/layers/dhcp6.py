...
class _DHCP6OptGuessPayload(Packet):
    ...
    def guess_payload_class(self, payload):
        ...
        if len(payload) > 2:
            ...
            cls = get_cls(dhcp6opts_by_code.get(
                opt, "DHCP6OptUnknown"), DHCP6OptUnknown)
        return cls
...
class DHCP6OptOptReq(_DHCP6OptGuessPayload):       # RFC sect 22.7
    ...
    fields_desc = [ShortEnumField("optcode", 6, dhcp6opts),
                   FieldLenField("optlen", None,
                                 length_of="reqopts", fmt="!H"),
...
class DHCP6OptIAPrefix(_DHCP6OptGuessPayload):  # RFC3633
    ...
    fields_desc = [ShortEnumField("optcode", 26, dhcp6opts),
                   ...
                   IntField("preflft", 0),
                   IntField("validlft", 0),
                   # TODO: Challenge that default value
                   ByteField("plen", 48),
                   # At least, global and won't hurt
   ...
   If you have a need for others, just ask ... or provide a patch."""
    ...
    def parse_options(self, dns="2001:500::1035", domain="localdomain, local",
        ...
        for o in [(dns, "dns", 23, lambda x: DHCP6OptDNSServers(dnsservers=x)),
                  ...
                  (sntpservers, "sntpservers", 31,
                   lambda x: DHCP6OptSNTPServers(sntpservers=x)),
                  (sipservers, "sipservers", 22,
                   lambda x: DHCP6OptSIPServers(sipservers=x)),
                  (sipdomains, "sipdomains", 21,
                   lambda x: DHCP6OptSIPDomains(sipdomains=x)),
                  (nisservers, "nisservers", 27,
                   lambda x: DHCP6OptNISServers(nisservers=x)),
                  (nisdomain, "nisdomain", 29,
                   lambda x: DHCP6OptNISDomain(nisdomain=(x + [""])[0])),
                  (nispservers, "nispservers", 28,
                   lambda x: DHCP6OptNISPServers(nispservers=x)),
                  (nispdomain, "nispdomain", 30,
                   lambda x: DHCP6OptNISPDomain(nispdomain=(x + [""])[0])),
                  (bcmcsservers, "bcmcsservers", 33,
                   lambda x: DHCP6OptBCMCSServers(bcmcsservers=x)),
    ...
    def make_reply(self, req):
        ...
        if msgtype == 1:  # SOLICIT (See Sect 17.1 and 17.2 of RFC 3315)
            ...
            if not p.haslayer(DHCP6OptClientId):
                if self.debug:
                    warning(
                        "Received Info Request message without Client Id option")
            else:
                client_duid = p[DHCP6OptClientId].duid
