...
class ISAKMPTransformSetField(StrLenField):
    ...
    def type2num(self, (typ, val)):
        ...
        if (val & ~0xffff):
            if not tlv:
                warning(
                    "%r should not be TLV but is too big => using TLV encoding" % typ)
            n = 0
            while val:
                s = chr(val & 0xff) + s
                val >>= 8
        ...
        while len(m) >= 4:
            ...
            if is_tlv:
                ...
                if value_len + 4 > len(m):
                    warning("Bad length for ISAKMP tranform type=%#6x" %
                            trans_type)
                value = m[4:4 + value_len]
                value = reduce(lambda x, y: (x << 8L) | y, struct.unpack(
                    "!%s" % ("B" * len(value),), value), 0)
            else:
                trans_type &= 0x7fff
                value_len = 0
...
class ISAKMP(ISAKMP_class):  # rfc2408
    ...
    fields_desc = [
        ...
        FlagsField("flags", 0, 8, ["encryption", "commit", "auth_only",
                                   "res3", "res4", "res5", "res6", "res7"]),  # XXX use a Flag field
        IntField("id", 0),
...
class ISAKMP_payload_Transform(ISAKMP_class):
    ...
    fields_desc = [
        ...
        ISAKMPTransformSetField(
            "transforms", None, length_from=lambda x: x.length - 8)
...
#    ISAKMP_payload_type = 0
    fields_desc = [
        ...
        PacketLenField("trans", conf.raw_layer(
        ), ISAKMP_payload_Transform, length_from=lambda x: x.length - 8),
    ]
...
class ISAKMP_payload_SA(ISAKMP_class):
    ...
    fields_desc = [
        ...
        PacketLenField("prop", conf.raw_layer(), ISAKMP_payload_Proposal,
                       length_from=lambda x: x.length - 12),
    ]
