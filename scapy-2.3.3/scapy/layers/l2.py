...
class GRE(Packet):
    ...
    fields_desc = [BitField("chksum_present", 0, 1),
                   ...
                   ConditionalField(XShortField(
                       "chksum", None), lambda pkt:pkt.chksum_present == 1 or pkt.routing_present == 1),
                   ConditionalField(XShortField(
                       "offset", None), lambda pkt:pkt.chksum_present == 1 or pkt.routing_present == 1),
                   ConditionalField(XIntField("key", None),
                                    lambda pkt:pkt.key_present == 1),
                   ConditionalField(XIntField("seqence_number", None),
                                    lambda pkt:pkt.seqnum_present == 1),
                   ]
...
# called LINUX_IRDA, similar to CookedLinux
conf.l2types.register(144, CookedLinux)
...
Set cache=True if you want arping to modify internal ARP-Cache"""
    ...
    if cache and ans is not None:
        for pair in ans:
            conf.netcache.arp_cache[pair[1].psrc] = (
                pair[1].hwsrc, time.time())
    if verbose:
        ans.show()
    return ans, unans
...
def etherleak(target, **kargs):
    ...
    return srpflood(Ether() / ARP(pdst=target),
                    prn=lambda (s, r): conf.padding_layer in r and hexstr(
                        r[conf.padding_layer].load),
                    filter="arp", **kargs)
