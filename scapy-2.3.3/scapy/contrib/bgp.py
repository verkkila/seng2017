...
class BGPHeader(Packet):
    ...
    fields_desc = [
        ...
        ShortField("len", None),
        ByteEnumField("type", 4, {0: "none", 1: "open",
                                  2: "update", 3: "notification", 4: "keep_alive"}),
    ]
...
class BGPOpen(Packet):
    ...
    fields_desc = [
        ...
        PacketListField("opt_parm", [], BGPOptionalParameter,
...
class BGPPathAttribute(Packet):
    ...
    fields_desc = [
        FlagsField("flags", 0x40, 8, ["NA0", "NA1", "NA2", "NA3", "Extended-Length",
                                      "Partial", "Transitive", "Optional"]),  # Extened leght may not work
        ByteEnumField("type", 1, {1: "ORIGIN", 2: "AS_PATH", 3: "NEXT_HOP",
                                  4: "MULTI_EXIT_DISC", 5: "LOCAL_PREF", 6: "ATOMIC_AGGREGATE", 7: "AGGREGATOR"}),
...
class BGPUpdate(Packet):
    ...
    fields_desc = [
        ...
        FieldListField("withdrawn", [], BGPIPField(
            "", "0.0.0.0/0"), length_from=lambda p:p.withdrawn_len),
        ShortField("tp_len", None),
        PacketListField("total_path", [], BGPPathAttribute,
                        length_from=lambda p: p.tp_len),
        FieldListField("nlri", [], BGPIPField("", "0.0.0.0/0"), length_from=lambda p:p.underlayer.len -
                       23 - p.tp_len - p.withdrawn_len),  # len should be BGPHeader.len
    ]
...
class BGPNotification(Packet):
    ...
    fields_desc = [
        ByteEnumField("ErrorCode", 0, {1: "Message Header Error", 2: "OPEN Message Error",
                                       3: "UPDATE Messsage Error", 4: "Hold Timer Expired", 5: "Finite State Machine", 6: "Cease"}),
        ByteEnumField("ErrorSubCode", 0, {
...
class BGPErrorSubcodes(Packet):
    ...
    Fields_desc = [
        ...
        ByteEnumField("UPDATEMessage", 0, {1: "Malformed Attribute List", 2: "Unrecognized Well-Known Attribute", 3: "Missing Well-Known Attribute", 4: "Attribute Flags Error", 5: "Attribute Length Error",
                                           6: "Invalid ORIGIN Attribute", 7: "AS Routing Loop", 8: "Invalid NEXT_HOP Attribute", 9: "Optional Attribute Error", 10: "Invalid Network Field", 11: "Malformed AS_PATH"}),
    ]
