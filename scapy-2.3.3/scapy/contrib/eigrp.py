...
class EigrpIP6Field(StrField, IP6Field):
    ...
    def m2i(self, pkt, x):
        ...
        if l > 128:
            warning(
                "EigrpIP6Field: Prefix length is > 128. Dissection of this packet will fail")
        else:
...
class EIGRPAuthData(EIGRPGeneric):
    ...
    def post_build(self, p, pay):
        ...
        if self.keysize is None:
            ...
            p = p[:6] + chr((keysize >> 8) & 0xff) + \
                chr(keysize & 0xff) + p[8:]
...
class ShortVersionField(ShortField):
    ...
    def i2repr(self, pkt, x):
        try:
            ...
            # We print a leading 'v' so that these values don't look like
            # floats
...
# one of the stub options. It's also the only flag set, when you configure
# "eigrp stub receive-only".
_EIGRP_STUB_FLAGS = ["connected", "static", "summary",
                     "receive-only", "redistributed", "leak-map"]
