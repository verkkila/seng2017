...
if conf.use_winpcapy:
    # mostly code from https://github.com/phaethon/scapy translated to
    # python2.X
    try:
                ...
                if p.contents.name.endswith(iff):
                    ...
                    while a:
                        if hasattr(socket, 'AF_LINK') and a.contents.addr.contents.sa_family == socket.AF_LINK:
                            ...
                            ret = str(val.contents.sdl_data[
                                      val.contents.sdl_nlen: val.contents.sdl_nlen + val.contents.sdl_alen])
                        a = a.contents.next
                    break
                p = p.contents.next
            return ret
                ...
                if p.contents.name.endswith(iff):
                    ...
                    while a:
                        if a.contents.addr.contents.sa_family == socket.AF_INET:
                            ...
                            ret = "".join([chr(x)
                                           for x in val.contents.sin_addr[:4]])
                        a = a.contents.next
                    break
                p = p.contents.next
            return ret
                ...
                while a:
                    if a.contents.addr.contents.sa_family == socket.AF_INET6:
                        ...
                        addr = socket.inet_ntop(
                            socket.AF_INET6, str(val.contents.sin6_addr[:]))
                        scope = scapy.utils6.in6_getscope(addr)
                        ret.append(
                            (addr, scope, p.contents.name.decode('ascii')))
                    a = a.contents.next
                p = p.contents.next
            return ret
        finally:
    ...
    class _PcapWrapper_pypcap:
        ...
        def __init__(self, device, snaplen, promisc, to_ms):
            ...
            self.pcap = pcap_open_live(
                self.iface, snaplen, promisc, to_ms, self.errbuf)
            self.header = POINTER(pcap_pkthdr)()
            self.pkt_data = POINTER(c_ubyte)()
            self.bpf_program = bpf_program()

        def next(self):
            c = pcap_next_ex(self.pcap, byref(
                self.header), byref(self.pkt_data))
            if not c > 0:
                return
            ts = self.header.contents.ts.tv_sec
            pkt = "".join([chr(i)
                           for i in self.pkt_data[:self.header.contents.len]])
...
if conf.use_pcap:
    ...
    try:
        ...
        try:
            ...
            if conf.interactive:
                log_loading.error(
                    "Unable to import pcap module: %s/%s" % (e, e2))
                conf.use_pcap = False
            else:
                raise
    if conf.use_pcap:
        ...
        if hasattr(pcap, "pcap"):  # python-pypcap
            class _PcapWrapper_pypcap:
                ...
                def next(self):
                    ...
                    ts, pkt = c
                    return ts, str(pkt)
            open_pcap = lambda *args, **kargs: _PcapWrapper_pypcap(
                *args, **kargs)
            ...
            class _PcapWrapper_libpcap:
                ...
                def __del__(self):
                    fd = self.pcap.fileno()
                    os.close(fd)
            open_pcap = lambda *args, **kargs: _PcapWrapper_libpcap(
                *args, **kargs)
            ...
            class _PcapWrapper_pcapy:
                ...
                def __del__(self):
                    warning(
                        "__del__: don't know how to close the file descriptor. Bugs ahead ! Please report this bug.")
            open_pcap = lambda *args, **kargs: _PcapWrapper_pcapy(
                *args, **kargs)
