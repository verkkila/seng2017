...
class _ATMT_supersocket(SuperSocket):
    ...
    def __init__(self, name, ioevent, automaton, proto, args, kargs):
        ...
        self.proto = proto
        self.spa, self.spb = socket.socketpair(
...
class Automaton_metaclass(type):
    ...
    def __new__(cls, name, bases, dct):
        ...
        classes = [cls]
        while classes:
            # order is important to avoid breaking method overloading
            c = classes.pop(0)
        ...
        for condname, actlst in cls.actions.iteritems():
            actlst.sort(lambda c1, c2: cmp(
                c1.atmt_cond[condname], c2.atmt_cond[condname]))

        for ioev in cls.iosupersockets:
            setattr(cls, ioev.atmt_as_supersocket, _ATMT_to_supersocket(
                ioev.atmt_as_supersocket, ioev.atmt_ioname, cls))
...
class Automaton:
    ...
    class InterceptionPoint(AutomatonStopped):
        ...
        def __init__(self, msg, state=None, result=None, packet=None):
            Automaton.AutomatonStopped.__init__(
                self, msg, state=state, result=result)
            self.packet = packet
        ...
        if self.state.state in self.interception_points:
            ...
            if cmd.type == _ATMT_Command.REJECT:
                ...
                raise self.AutomatonError(
                    "INTERCEPT: unkown verdict: %r" % cmd.type)
        self.my_send(pkt)
    ...
    def run(self, resume=None, wait=True):
        ...
        if wait:
            ...
            if c.type == _ATMT_Command.END:
                ...
                raise self.InterceptionPoint(
                    "packet intercepted", state=c.state.state, packet=c.pkt)
            elif c.type == _ATMT_Command.SINGLESTEP:
                raise self.Singlestep(
                    "singlestep state=[%s]" % c.state.state, state=c.state.state)
            elif c.type == _ATMT_Command.BREAKPOINT:
                raise self.Breakpoint(
                    "breakpoint triggered on state [%s]" % c.state.state, state=c.state.state)
