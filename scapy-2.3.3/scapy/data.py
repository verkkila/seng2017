...
# Added to have more specific info (should be 0x0101 ?)
    ...
    try:
        for l in open(filename):
            try:
                ...
                log_loading.info(
                    "Couldn't parse file [%s]: line [%r] (%s)" % (filename, l, e))
    except IOError:
    ...
    try:
        ...
        for l in f:
            try:
                ...
                log_loading.warning(
                    "Couldn't file [%s]: line [%r] (%s)" % (filename, l, e))
        f.close()
    except IOError:
...
if WINDOWS:
    ...
    IP_PROTOS = load_protocols(
        os.environ["SystemRoot"] + "\system32\drivers\etc\protocol")
